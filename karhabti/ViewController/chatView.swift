...
class ChatView: MessagesViewController, MessagesDataSource, MessagesLayoutDelegate, MessagesDisplayDelegate {
    ...
    override func viewDidLayoutSubviews() {
        messageInputBar.sendButton.addAction(UIAction(handler: { [self] UIAction in
            ...
            MessagerieContoller.sharedInstance.envoyerMessage(recepteur: (currentConversation?.receiver._id)!, description: messageDescription) { success, messageFromRep in
                if success {
                    let currentUser = (currentConversation?.sender)!
                    var message = messageFromRep!
                    message.sender = Sender(senderId: currentUser._id!, displayName: currentUser.nom!)
    ...
    func currentSender() -> SenderType {
        let currentUser = (currentConversation?.sender)!
        return Sender(senderId: currentUser._id!, displayName: currentUser.nom!)
    }
...
//        printContent(currentConversation)
        MessagerieContoller.sharedInstance.recupererMesMessages(idConversation: (currentConversation?._id)!) { [self] success, messagesFromRep in
            if success {
